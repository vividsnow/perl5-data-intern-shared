...
my ($mnt, @args) = @ARGV;
my $h = eval { Data::Intern::Shared->new("$mnt/x.shm", @args) };
print $h ? "created\n" : "refused: $@";
P
close $fh;
...
($sig, $out) = on_small_tmpfs('DATA_INTERN_SHARED_SPARSE=0', 1000);
is $sig, 0, 'DATA_INTERN_SHARED_SPARSE=0: a segment that fits: no signal';
like $out, qr/^created/, '  and it is created';

# Construction writes only the header and the 2 MiB hash table, not the reverse array or the arena.
($sig, $out) = on_small_tmpfs('DATA_INTERN_SHARED_SPARSE=0', 100_000);
is $sig, 0, 'DATA_INTERN_SHARED_SPARSE=0: a segment the filesystem cannot hold does not kill the process';
like $out, qr/^refused: .*No space left on device/, '  it is refused with the reason';
diag $out unless $out =~ /^refused/;

($sig, $out) = on_small_tmpfs('', 100_000);
